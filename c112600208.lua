...
function s.rthcfilter(c,tp)
	return c:IsFaceup() and c:IsSetCard(0xe8b) and c:IsAbleToHandAsCost()
		and Duel.IsExistingTarget(s.rthtgfilter,tp,0,LOCATION_ONFIELD,1,c,c)
end
function s.rthtgfilter(c,tc)
...
function s.rthtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then
		if e:GetLabel()==1 then
			e:SetLabel(0)
			return Duel.IsExistingMatchingCard(s.rthcfilter,tp,LOCATION_ONFIELD,0,1,c,tp)
		else
	...
	if e:GetLabel()==1 then
		...
		local g=Duel.SelectMatchingCard(tp,s.rthcfilter,tp,LOCATION_ONFIELD,0,1,1,c,tp)
