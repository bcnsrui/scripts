...
function cm.afilter(c)
	return c:IsFaceup() and c:IsCode(112604166)
end
function cm.afilter2(c,e,tp)
	return (c:IsSetCard(0xe92) or c:IsSetCard(0xe78) or c:IsSetCard(0xe9f)) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function cm.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 then
		...
		if Duel.IsExistingMatchingCard(cm.afilter,tp,LOCATION_ONFIELD,0,1,nil) then
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
