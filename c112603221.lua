...
function cm.initial_effect(c)
	...
	--MAX HYPER UP
	Xyz.AddProcedure(c,cm.mfilter,8,2)
	c:EnableReviveLimit()
...
function cm.mfilter(c,lc,sumtype,tp)
	return c:IsCode(112603207) or c:IsCode(112603205)
end
...
function cm.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsOnField() and cm.filter0(chkc) end
	if chk==0 then return Duel.IsExistingTarget(cm.filter0,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RTOHAND)
	local g=Duel.SelectTarget(tp,cm.filter0,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil)
...
function cm.filter1(c,e,tp)
	return c:IsFaceup() and c:IsSetCard(0xe9c) and not c:IsAttack(2600) and c:IsLevel(8) and c:CheckUniqueOnField(tp) and c:IsCanBeSpecialSummoned(e,0,tp,false,false,POS_FACEUP_DEFENSE)
end
