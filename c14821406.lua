...
function c14821406.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return not c:IsPublic() and c:GetFlagEffect(14821406)==0 end
	c:RegisterFlagEffect(14821406,RESET_CHAIN,0,1)
end
function c14821406.filter(c)
...
function c14821406.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		...
		and Duel.IsExistingTarget(c14821406.filter,tp,LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	Duel.SelectTarget(tp,c14821406.filter,tp,LOCATION_MZONE,0,1,1,nil)
...
function c14821406.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) then
		if Duel.SpecialSummonStep(c,0,tp,tp,false,false,POS_FACEUP) then
			if tc:IsRelateToEffect(e) and tc:IsFaceup() and Duel.SelectEffectYesNo(tp,e:GetHandler(),aux.Stringid(14821406,1)) then
				Duel.BreakEffect()
...
function c14821406.disop(e,tp,eg,ep,ev,re,r,rp)
	...
	e1:SetDescription(aux.Stringid(14821406,0))
