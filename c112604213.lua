...
function s.initial_effect(c)
	...
	e8:SetDescription(1068)
	e8:SetCategory(CATEGORY_EQUIP)
	e8:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e8:SetType(EFFECT_TYPE_IGNITION)
	e8:SetRange(LOCATION_GRAVE+LOCATION_MZONE) --changed here
	e8:SetTarget(Auxiliary.UnionTarget(s.unfilter,nil))
	e8:SetOperation(Auxiliary.UnionOperation(s.unfilter))
	c:RegisterEffect(e8)
	--unequip
	...
	e9:SetCondition(function(e) return e:GetHandler():GetEquipTarget() end)
	e9:SetTarget(Auxiliary.UnionSumTarget(nil))
	e9:SetOperation(Auxiliary.UnionSumOperation(nil))
	c:RegisterEffect(e9)
	--destroy sub
	...
	e10:SetCondition(function(e) return e:GetHandler():GetEquipTarget() end)
	e10:SetValue(Auxiliary.UnionReplace(nil))
	c:RegisterEffect(e10)
	--eqlimit
	local e11=Effect.CreateEffect(c)
	e11:SetType(EFFECT_TYPE_SINGLE)
	e11:SetCode(EFFECT_UNION_LIMIT)
	e11:SetProperty(EFFECT_FLAG_CANNOT_DISABLE)
	e11:SetValue(Auxiliary.UnionLimit(s.unfilter))
