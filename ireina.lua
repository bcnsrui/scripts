...
function Card.RegisterEffect(c,e,forced,...)
	...
	elseif code==10045474 and mt.eff_ct[c][0]==e then
		local filter=function(c,tp)
			return c:IsFaceup()
	...
	elseif code==112603120 and mt.eff_ct[c][0]==e then
		local filter=function(c,tp)
			return c:IsFaceup()
				and ((not c:IsDisabled() and c:IsType(TYPE_EFFECT) and not c:IsCode(m)
	...
	elseif code==112603120 and mt.eff_ct[c][1]==e then
		local filter=function(c,tp)
			return c:IsFaceup()
				and ((not c:IsDisabled() and not c:IsType(TYPE_NORMAL) and not c:IsCode(m)
					and c:IsControler(1-tp)	and c:IsLocation(LOCATION_GRAVE+LOCATION_REMOVED))
					or c.delightsworn)
		end
		e:SetTarget(function(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
