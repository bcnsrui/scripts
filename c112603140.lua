...
function cm.nfil11(c,fc,sub,concat_fusion)
	...
	return c:IsCode(table.unpack(cm.fusion_materials)) or (sub and c:CheckFusionSubstitute(fc))
end
function cm.nfil12(c,tp,fc,sub,chkfnf,mg,sg,...)
	...
	if t[2] then
		...
		local res=(concat_fusion or not sg:IsExists(Auxiliary.TuneMagicianCheckX,1,nil,sg,EFFECT_TUNE_MAGICIAN_F))
			and (c:IsCode(t[1]) or (sub and c:CheckFusionMaterial(fc)))
...
function cm.pfil1(c,fc)
	return (c:IsCode(table.unpack(cm.fusion_materials)) or (sub and c:CheckFusionSubstitute(fc))) and c:IsAbleToRemoveAsCost()
end
