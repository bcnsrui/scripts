...
function cm.tfil0(c)
	return c:IsFaceup() and c:IsPreviousLocation(0x40) and c:IsSummonType(SUMMON_TYPE_SPECIAL)
end
...
function cm.op2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local g=Duel.GetMatchingGroup(cm.tfil0,tp,LOCATION_MZONE,LOCATION_MZONE,nil)
	local tc=g:GetFirst()
	while tc do
		local e1=Effect.CreateEffect(c)
