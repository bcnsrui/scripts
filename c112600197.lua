...
function s.initial_effect(c)
	...
	--fusion material
	c:EnableReviveLimit()
	Fusion.AddProcMix(c,true,true,112600184,112600185,112600186,112600187)
	...
	e9:SetCondition(s.discon0)
	e9:SetCost(s.discost0)
	e9:SetTarget(s.distg0)
	e9:SetOperation(s.disop0)
	c:RegisterEffect(e9)
	--remove
...
function s.imvl(e,te)
	return te:IsActiveType(TYPE_MONSTER)
	   and te:GetOwnerPlayer()~=e:GetHandlerPlayer()
end
function s.discon0(e,tp,eg,ep,ev,re,r,rp)
	return re:GetHandler()~=e:GetHandler() and not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED)
		and (re:IsActiveType(TYPE_MONSTER) or re:IsHasType(EFFECT_TYPE_ACTIVATE)) and Duel.IsChainNegatable(ev)
end
function s.discost0(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsDiscardable,tp,LOCATION_HAND,0,1,nil) end
	Duel.DiscardHand(tp,Card.IsDiscardable,1,1,REASON_COST+REASON_DISCARD)
end
function s.distg0(e,tp,eg,ep,ev,re,r,rp,chk)
...
function s.disop0(e,tp,eg,ep,ev,re,r,rp)
