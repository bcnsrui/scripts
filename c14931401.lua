...
function s.spfilter(c,e,tp)
	return c:IsCode(14931402) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function s.filter(c)
	return c:IsSetCard(0xb93) and c:IsSynchroSummonable(nil)
end
...
function s.filter1(c)
	return c:IsSetCard(0xb93) and c:IsXyzSummonable(nil)
end
function s.tg1(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.filter1,tp,LOCATION_EXTRA,0,1,nil,nil) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_EXTRA)
end
function s.op1(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(s.filter1,tp,LOCATION_EXTRA,0,nil,nil)
	if #g>0 then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local sg=g:Select(tp,1,1,nil)
